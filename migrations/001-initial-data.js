...
module.exports = {
  up: async (mongoose) => {
    ...
    const Client = mongoose.models.Client || mongoose.model('Client', new Schema({
      name: { type: String, required: true },
    ...
    const Sale = mongoose.models.Sale || mongoose.model('Sale', new Schema({
    ...
    const HeroSlide = mongoose.models.HeroSlide || mongoose.model('HeroSlide', new Schema({
    ...
    const PaymentMethod = mongoose.models.PaymentMethod || mongoose.model('PaymentMethod', new Schema({
      name: { type: String, required: true },
    ...
    const SiteConfig = mongoose.models.SiteConfig || mongoose.model('SiteConfig', new Schema({
    ...
    const paymentMethods = [
      {
        ...
        order: 0,
        config: { iban: 'IT00 X000 0000 0000 0000 0000 000', beneficiary: 'Nicole Trend Shop S.r.l.' },
      },
      {
        name: 'PayPal',
        type: 'paypal',
        ...
        order: 1,
        config: { email: 'payments@nicoletrend.com' },
      },
      {
        name: 'Contrassegno',
    ...
    for (const method of paymentMethods) {
      const exists = await PaymentMethod.findOne({ type: method.type });
      if (!exists) {
    ...
    for (const config of siteConfigs) {
      const exists = await SiteConfig.findOne({ key: config.key });
      if (!exists) {
