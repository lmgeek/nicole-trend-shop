...
export default function ModificaHeroPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/hero-slides/${id}`).then((r) => r.json()),
    ]).then(([prods, slideData]) => {
      setProducts(Array.isArray(prods) ? prods : []);
      setSlide(slideData);
      setFormData({
        type: slideData.type || 'custom',
        productId: slideData.product?._id || '',
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500">Caricamento slide...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Modifica Hero Slide</h1>
          <p className="text-gray-500 text-sm mt-1">{slide?.title || slide?.product?.name}</p>
        </div>
      </div>
